...
export default function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<'whatsapp' | 'paystack'>('whatsapp');
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        payment_method: paymentMethod === 'whatsapp' ? 'WhatsApp' : 'Paystack',
        total_amount: total,
        status: 'pending',
        items: cart.map(item => ({
          product_id: item.id,
      ...
      const createResponse = await fetch('/api/public/orders', {
        method: 'POST',
      ...
      if (paymentMethod === 'paystack') {
        ...
        const initResponse = await fetch('/api/payment/initialize', {
          ...
          body: JSON.stringify({
            ...
            amount: total,
            reference: orderData.order_number // Use order number as reference
          })
        });
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-24 sm:py-32 text-center animate-in fade-in zoom-in-95 duration-700">
        ...
        <p className="text-gray-500 mb-12 text-lg leading-relaxed max-w-lg mx-auto">
          Thank you for your purchase. Mia has received your order and is currently processing it.
        </p>
        <div className="bg-gray-50 p-8 rounded-sm border border-gray-100 mb-12 text-left max-w-md mx-auto">
          <div className="flex justify-between text-xs uppercase tracking-[0.2em] font-bold text-gray-400 mb-4">
            ...
            <span className="text-black">#{orderNumber || 'PENDING'}</span>
          </div>
          ...
          <section>
            ...
            <div className="space-y-4">
              <div 
                ...
                className={`border p-6 rounded-sm flex items-center justify-between cursor-pointer transition-all ${paymentMethod === 'whatsapp' ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300'}`}
              >
                <div className="flex items-center gap-4">
                  <MessageCircle className={`w-6 h-6 ${paymentMethod === 'whatsapp' ? 'text-emerald-500' : 'text-gray-400'}`} />
                  <div>
                ...
                <div className={`w-5 h-5 border-4 rounded-full flex items-center justify-center ${paymentMethod === 'whatsapp' ? 'border-black' : 'border-gray-200'}`}>
                  {paymentMethod === 'whatsapp' && <div className="w-2 h-2 bg-black rounded-full"></div>}
                </div>
              </div>
              
              <div 
                onClick={() => setPaymentMethod('paystack')}
                className={`border p-6 rounded-sm flex items-center justify-between cursor-pointer transition-all ${paymentMethod === 'paystack' ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300'}`}
              >
                <div className="flex items-center gap-4">
                  <CreditCard className={`w-6 h-6 ${paymentMethod === 'paystack' ? 'text-black' : 'text-gray-400'}`} />
                  <div>
                ...
                <div className={`w-5 h-5 border-4 rounded-full flex items-center justify-center ${paymentMethod === 'paystack' ? 'border-black' : 'border-gray-200'}`}>
                  {paymentMethod === 'paystack' && <div className="w-2 h-2 bg-black rounded-full"></div>}
                </div>
              </div>
            </div>
          </section>
